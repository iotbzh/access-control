...
def log_access(reader_id, badge_uid, user_name, user_id, guest, result, reason):
    guest = guest.strip("- ") or None if guest else ''
    try:
        dbs.add(Log(reader_id=reader_id, badge_uid=badge_uid, user_name=user_name, user_id=user_id, guest=guest, result=result, reason=reason))
        dbs.commit()
        Logger.get_app("access").info(f"[ {datetime.now()} ] {user_name} {guest} ({badge_uid}) {result} on reader {reader_id} ({reason})")
...
def access_control(gateway, reader_instance, badge_uid):
    ...
    if not reader.is_active:
        log_access(reader.id, badge_uid, None, None, None, "denied", "Inactive Reader")
    ...
    if not badge:
        log_access(reader.id, badge_uid, None, None, None, "denied", "Invalid Badge")
    ...
    if not role.is_active:
        log_access(reader.id, badge_uid, user.name, user.id, f"{badge.guest_name or ''} - {badge.company_name or ''}", "denied", "Inactive role")
        logging.debug("Access refused. Inactive role.")
        gateway.event(reader_instance, False, badge_uid)
        return False
