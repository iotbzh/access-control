...
def index():
    ...
    query = db.select(Reader)
    if is_active in ("0", "1"):
        query = query.where(Reader.is_active == is_active)
    
    readers = dbs.execute(query).scalars().all()
    return render_template('readers/index.html', is_active=is_active, readers=readers, readers_instance=BaseGateway.readers)
