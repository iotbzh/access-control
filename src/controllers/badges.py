...
from src.models import db, dbs, Badge, User
...
def add():
    if request.method == 'POST':
        ...
        users = User.query.all()
        return render_template('badges/add.html', users=users)
...
def edit(badge_id):
    if request.method == 'POST':
        ...
        dbs.execute(db.update(Badge).where(Badge.id == badge_id).values(uid=uid, user_id=user_id, is_active=is_active, deactivation_date=deactivation_date))
        dbs.commit()
        ...
        users = User.query.all()
        return render_template('badges/edit.html', badge=badge, users=users)
